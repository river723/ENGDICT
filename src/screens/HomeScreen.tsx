...
type SuggestionRoute = 'Study' | 'AddWord';
...
const buildTodaySuggestion = (stats: TodayStats): TodaySuggestion => {
  ...
  if (stats.todayStudyCount >= 3 && stats.accuracy < 0.6) {
    ...
    return {
      title: '今天正确率偏低',
      description: `当前正确率约 ${Math.round(stats.accuracy * 100)}%，建议先复习困难词。`,
  ...
  if (stats.todayTotal > 0 && stats.todayPending === 0) {
    return {
      title: '今日任务已完成',
      description: '学习节奏不错，可以再做一组卡片巩固一下。',
      actionLabel: '继续学习',
      icon: 'book-open-variant',
      routeName: 'Study',
    };
...
export default function HomeScreen() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [allWords, allPlans, todayRecords, allRecords] = await Promise.all([
